...
#REGISTRY_IMAGE='my-rmi-registry'
...
docker container stop $REGISTRY_CONTAINER 2> /dev/null && docker container rm $REGISTRY_CONTAINER 2> /dev/null
...
#docker build -t $REGISTRY_IMAGE --target rmiregistry-build .